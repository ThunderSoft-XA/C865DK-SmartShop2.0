...
import com.thundercomm.eBox.AI.AgeGenderDectorTask;
...
public class MyGLSurfaceView extends GLSurfaceView {
    ...
    private void getAlgorithmType() {
        if (getAlgorithmType(surfaceId, Constants.ENABLE_AGEGENDER_STR)) {
            p = new PutIntoAgeGenderDectorTask();
        } else {
            p = null;
        }
    }
    ...
    private void getObjectInfo(int surfaceId, Mat mat) {
        //todo: 上传
        AgeGenderDectorTask.getAgeGenderDectorTask().addMatById(surfaceId, mat, mOutputWidth, mOutputHeight);
    }
     ...
     * 实现接口送年龄性别识别
     * */
    private class PutIntoAgeGenderDectorTask implements PutIntoTask {
        private long pts = 0;

        @Override
        public void putInTask(int surfaceId, Mat matFromGlSurface, GL10 arg0) {
            if (AgeGenderDectorTask.getAgeGenderDectorTask().isIstarting()) {
                if (System.currentTimeMillis() - pts >= Constants.INPUT＿AGEGENDER_INTERVAL) {
